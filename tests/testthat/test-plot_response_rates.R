...
valid <- unlist(lapply(1:n_participants, function(participant) {
       rep(c(rep(TRUE, participant), rep(FALSE, beeps_per_day - participant)), n_days)
   }))
...
data <- data.frame(
  ...
  valid = valid,
  dates = rep(seq(as.Date("2023-01-01"), as.Date("2023-01-01") + (n_days-1), by = "day"),
...
test_that('response_rate_per_day works correctly', {
  ...
  for(participant in 1:n_participants){
    ...
    expect_equal(unique(data_plot[data_plot$participant == participant, ]$response_rate), expected_rate)
  }
})
